...
const deleteRole = async (req, res) => {
    try {
        const { id } = req.params;
        const role = await db.Role.findByPk(id);
        const rolePermissions = await db.Role_Permission.findAll({ where: { role_id: id } });
        if (role) {
            if (rolePermissions) {
...
const assignPermissionToRole = async (req, res) => {
    try {
        ...
        const rolePermissions = await db.Role_Permission.findOne({ where: { role_id: id, permission_id: permission_id } });
        if (rolePermissions) {
            return 
        }
        await db.Role_Permission.create({role_id: id, permission_id });
        return res.status(201).json({ message: 'Permission assigned to role successfully' });
    } catch (error) {
        return res.status(500).json({ error: 'Failed to assign permission to role' });
    }
}
...
const removePermissionFromRole = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: 'Permission deleted from role successfully' });
    } catch (error) {
        return res.status(500).json({ error: 'Failed to delete permission from role' });
    }
}
const getRolePermissions = async (req, res) => {
    try {
        ...
        return res.status(500).json({ error: 'Failed to retrieve role permissions' });
    }
}
module.exports = {
    createRole,
