...
const getReceiptNoteById = async (req, res) => {
    try {
        const {id}=req.params
        const receiptNote = await db.ReceiptNote.findOne({ where: { receiptNote_id: id } });
        if(!receiptNote){
            return res.status(404).json({ message: `no receipt found with id ${req.params.id}!` });
        }
        return res.status(201).json({ receiptNote });
    }
    catch (error) {
...
const createReceiptNote = async (req, res) => {
    try {
        ...
        const createdReceiptNote = await db.ReceiptNote.create({deliveryDate:deliveryDate,comment:comment,command_id:command_id,type:type,command_id:id,type:type});
