...
const deleteBranch = async (req, res) => {
    try {
        ...
        const branchProducts= await db.Product.findAll({where: {branch_id: id}});
        if(branchProducts){
...
const assignBranchToChapter = async (req, res) => {
    try {
        ...
        if(branch.chapter_id==chapter_id){
            return res.status(400).json({ error: 'Branch already assigned to this chapter' });
        }
        branch.chapter_id = chapter_id;
        await branch.save();
        return res.status(200).json({ message: 'Branch assigned to chapter successfully' });
    } catch (error) {
        return res.status(500).json({ error: 'Failed to assign branch to chapter' });
    }
}
const removeBranchFromChapter = async (req, res) => {
    try {
        const { id } = req.params;
        const { chapter_id } = req.body;
        const branch = await db.Branch.findOne({where: {branch_id: id, chapter_id: chapter_id}});
        if (!branch || branch.length===0) {
            return res.status(404).json({ error: 'Branch not found' });
        }
        branch.chapter_id = null;
        await branch.save();
        return res.status(200).json({ message: 'Branch removed from chapter successfully' });
    } catch (error) {
        return res.status(500).json({ error: 'Failed to remove branch from chapter' });
    }
}
...
const getBranchProducts = async (req, res) => {
    try {
        ...
        if(!products || products.length===0) {
            return res.status(404).json({ error: 'No products found for this branch' });
        }
